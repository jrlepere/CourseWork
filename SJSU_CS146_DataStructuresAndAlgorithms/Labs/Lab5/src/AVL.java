...
public class AVL<AnyType extends Comparable<? super AnyType>> {
	 ...
	 */
	public <T extends Comparable<? super T>> boolean isSimilar(AVL<T> other) {
	...
	private AVLNode<AnyType> rightLeftRotation(AVLNode<AnyType> k3) {
		
		AVLNode<AnyType> k2 = k3.right.left;
		k3.right.left = k2.right;
		k2.right = k3.right;
		k3.right.height = Math.max(height(k3.right.left.height), height(k2.right.right.height));
		k2.height = Math.max(height(k2.height), height(k2.height));
		k3.right = k2;
		
		AVLNode<AnyType> k1 = k3.right;
		k3.right = k1.left;
		k1.right = k3;
		k3.height = Math.max(height(k3.left.height), height(k3.right.height));
		k1.height = Math.max(height(k1.left.height), height(k1.right.height));
	...
	private AVLNode<AnyType> leftRightRotation(AVLNode<AnyType> k3) {
		...
		AVLNode<AnyType> k2 = k3.left.right;
		k3.left.right = k2.left;
		k2.left = k3.left;
		k3.left.height = Math.max(height(k3.left.left.height), height(k2.left.right.height));
		k2.height = Math.max(height(k2.height), height(k2.height));
		k3.left = k2;
